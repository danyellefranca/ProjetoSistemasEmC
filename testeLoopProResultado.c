...
int main()
{
   ...
   for (i = 0; i < numLinhas; i++) // percorre a matriz, linha a linha
   {
       ...
       for (l = 0; l < numLinhas; l++) // percorre a matriz, linha por linha
       {
           ...
           {
               /*
               Copiar a linha do elemento que tornamos 1 em um vetor (FEITO - LINHA ATUAL). tornar o restante da 
               coluna igual a zero, 
               pegando o número que quero tornar zero (FEITO), trocando o sinal (FEITO) desse número e 
               multiplicando pela linha 
               que copiamos. depois, somamos os valores obtidos pelos respectivos valores (da mesma posição) na 
               linha que queremos tornar o elemento igual a 0. MAS N ALTERAR A LINHA ORIGINAL QUE TEM O ELEMENTO 
               QUE TORNAMOS 1
               */
